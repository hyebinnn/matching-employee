"""매칭 품질 평가: 목업 이력서에 붙여둔 의도 유형(완벽/부분/미스매칭) 순서가 지켜지는지 센다.
    ...
    uv run python -m tools.evaluate --sweep      # 임계값 조합 비교 (임베딩 판정만, API 호출 없음)
...
def load_labels(job_id: str) -> dict[str, str]:
    path = Path(f"data/expected/{job_id}.json")
    data = json.loads(path.read_text(encoding="utf-8"))["candidates"]
...
def evaluate(service: MatchingService, repo: JsonRepository) -> list[tuple]:
    results = []
...
def report(results: list[tuple]) -> None:
    for job, ranked, labels, bad, gap in results:
        print(f"\n== {job.title} ({job.id}) — 순위 역전 {bad}회, 완벽-미스매칭 간격 {gap:.1f}점")
...
def sweep(config: ScoringConfig, repo: JsonRepository) -> None:
    ...
    for met, partial in product(MET_CANDIDATES, PARTIAL_CANDIDATES):
        ...
        results = evaluate(build_service(tuned, use_llm=False), repo)
        rows.append((sum(r[3] for r in results), -min(r[4] for r in results), met, partial))
...
def main() -> None:
    ...
    if args.sweep:
        sweep(config, repo)
        return
    report(evaluate(build_service(config, use_llm=not args.no_llm), repo))
