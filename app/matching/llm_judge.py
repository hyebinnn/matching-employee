"""애매 구간만 LLM에게 다시 묻는 판정기.

임베딩은 표현이 다른 같은 경험(예: "화장품 단상자 그래픽 설계" ↔ "패키지 디자인 및 감리")을
잘 잡지 못한다. 유사도가 확실히 높지도 낮지도 않은 구간만 LLM에게 이력서 전문과 함께 물어
판정을 덮어쓴다. 점수 계산은 여전히 코드가 한다 — LLM은 status와 근거 문장만 돌려준다.
...
class LlmJudge:
    """임베딩 판정기를 감싸, 애매 구간 판정만 LLM 결과로 교체한다."""
